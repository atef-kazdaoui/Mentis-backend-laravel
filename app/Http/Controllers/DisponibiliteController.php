...
class DisponibiliteController extends Controller
{
    ...
    public function store(Request $request, $menthorId)
    {
        ...
        // Vérifier si le Menthor existe dans la base de données
        $menthor = Menthor::find($menthorId);
        if (!$menthor) {
            return response()->json(['error' => 'Menthor not found'], 404);
        }
    
        // Créer une nouvelle disponibilité pour le Menthor
        $disponibilite = new Disponibilite();
        $disponibilite->menthor_id = $menthorId; // Utilise l'ID passé dans l'URL
